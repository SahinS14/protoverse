...
function updateActiveSatList() {
    ...
    if (ids.length === 0) {
        list.innerHTML = `<div class="text-center small mt-4"><i class="fas fa-satellite fa-2x mb-2 opacity-25"></i><br>No satellite selected yet.</div>`;
        return;
    }
...
async function runScreening() {
    showLoading(true, "Screening Started...");
    try {
...
async function runSSAAnalysis() {
    showLoading(true, "AI Classification in Progress...");
    try {
        const res = await fetch(`${API_BASE}/ssa/run-analysis`, { method: 'POST' });
        const data = await res.json();
        alert(`Analysis Complete! ${data.processed_satellites} satellites classified.`);
...
// CLUSTER MEANINGS ---
const SSA_REGIMES = {
    0: { name: "LEO - Low Earth Orbit (High Traffic)", color: "#00f3ff", icon: "fa-layer-group" },
    1: { name: "MEO - Medium Earth Orbit (Navigation/GPS)", color: "#ffae00", icon: "fa-satellite-dish" },
    2: { name: "GEO - Geostationary (Communication Belt)", color: "#bc13fe", icon: "fa-broadcast-tower" },
    3: { name: "HEO - High Elliptical (Military/Strategic)", color: "#0aff60", icon: "fa-shield-alt" },
    4: { name: "VLEO - Very Low (Atmosphere Boundary)", color: "#ff0055", icon: "fa-meteor" }
};
...
async function fetchAndRenderSSAResults() {
    ...
    try {
        ...
        data.forEach(item => {
            ...
            const regime = SSA_REGIMES[item.cluster_id] || { name: "Undefined Region", color: "#666", icon: "fa-question" };
            const riskClass = item.decay_risk === 'CRITICAL' ? 'bg-danger animate__animated animate__flash animate__infinite' :
                (item.decay_risk === 'MEDIUM' ? 'bg-warning text-dark' : 'bg-success opacity-50');
                    ...
                    <tr class="animate__animated animate__fadeIn">
                        <td>
                            ...
                            <div class="small text-info opacity-75">
                              ${item.predicted_country === 'Unknown' ? '' : (" " + item.predicted_country || '')}
                            </div>
    ...
    } catch (e) { console.error("SSA Table Error:", e); }
}
    ...
    try {
        ...
        if (dbStatsContainer) {
            dbStatsContainer.innerHTML = `
                    <div class="d-flex justify-content-between mb-1"><span>Data Source:</span><span class="text-info font-mono">UCS Database</span></div>
                    <div class="d-flex justify-content-between mb-1"><span>Total Classes:</span><span class="text-info font-mono">${data.classes.length} Units</span></div>
                    <div class="d-flex justify-content-between mb-1"><span>Features:</span><span class="text-info font-mono">${Object.keys(data.feature_importance).length} Parameters</span></div>
                    <div class="d-flex justify-content-between"><span>Training Date:</span><span class="text-info font-mono" style="font-size:0.6rem;">${new Date(data.timestamp).toLocaleString()}</span></div>
                `;
        }
        ...
        if (reportBody) {
            ...
            Object.entries(data.classification_report).forEach(([className, metrics]) => {
                // Skip general rows like 'accuracy', 'macro avg', only take classes
        ...
        if (cmContainer) {
            ...
            data.confusion_matrix.forEach((row, i) => {
                ...
                row.forEach((val, j) => {
                    // Heatmap color (Correct predictions in blue tones, wrong ones reddish)
        ...
        console.error("Metric Loading Error:", e);
        alert("An error occurred while loading the technical report. Please train the model first.");
    }
